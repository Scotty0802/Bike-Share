...
```{r}
#Penalized Model
pen_model <- linear_reg(penalty=tune(),mixture=tune()) %>% set_engine("glmnet")
...
pen_grid_params <- grid_regular(penalty(), mixture(), levels = 10) ## L^2 total tuning possibilities
...
## Run the CV
CV_pen <- pen_wf %>%
tune_grid(resamples=folds,grid=pen_grid_params,metrics=metric_set(rmse, mae)) #Or leave metrics NULL

tune_tree <- tune_grid(
  tree_wf,
  resamples = folds,
  grid = tree_grid,
  metrics = metric_set(rmse, rsq) # regression metrics
)

## Plot Results (example)
collect_metrics(CV_pen) %>% # Gathers metrics into DF
filter(.metric=="rmse") %>%
ggplot(data=., aes(x=penalty, y=mean, color=factor(mixture))) +
geom_line()

collect_metrics(tune_tree) %>% # Gathers metrics into DF
filter(.metric=="rmse") %>%
...
test_preds_pen <- predict(final_wf, new_data = test)
test_preds_pen <- test_preds_pen |>
  mutate(.pred = exp(.pred))

test_preds_tree <- predict(final_wf_tree, new_data = test)
test_preds_tree <- test_preds_tree |>
  mutate(.pred = exp(.pred))
```
...
```{r}
kaggle_submission_pen <- test_preds_pen %>%
bind_cols(., test) %>% 
...
kaggle_submission_tree <- test_preds_tree %>%
bind_cols(., test) %>% 
  select(datetime, .pred) |>
  rename(count=.pred) %>%  
  mutate(count=pmax(0, count)) %>% 
