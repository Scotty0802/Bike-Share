---
title: "Bike_Share"
format: pdf
editor: visual
---

## Set up

```{r}
#| output: false
#| message: false
library(tidyverse)
library(tidymodels)
library(vroom)
library(ggplot2)
library(dplyr)
library(patchwork)
library(glmnet)
library(rpart)
library(bonsai)
library(lightgbm)
library(agua)
```

## Import

```{r}
#| output: false
#| message: false

train<- vroom("C:/Users/riley/Desktop/Fall 2025/Stat 348/data/bike-sharing-demand/train.csv")
test <- vroom("C:/Users/riley/Desktop/Fall 2025/Stat 348/data/bike-sharing-demand/test.csv")
```

## Clean

```{r}
train <- train |>
  #mutate(workingday = as.factor(workingday),
   #      holiday = as.factor(holiday)) |>
  #mutate(weather = factor(weather), 
  #                         levels = c("1","2","3","4"),
  #                         labels = c("Clear to Cloudy","Very Cloudy","Percipitation","Heavy Percipitation"))) |>
  # mutate(season = factor(season,
  #                        levels = c("1","2","3","4"),
  #                        labels = c("Spring","Summer","Fall","Winter"))) |>
  select(-casual, -registered)|>
  mutate(count = log(count))
```

## Model Making

```{r}
#Stacked Model
Sys.setenv(JAVA_HOME="C:/Program Files/Eclipse Adoptium/jdk-25.0.0.36-hotspot")


h2o::h2o.init()

model<- auto_ml() |>
set_engine("h2o", max_models=8) |>
set_mode("regression")

```

## Recipe

```{r}
bike_recipe <- recipe(count ~ ., data = train) %>%
  step_mutate(weather = ifelse(weather == 4, 3, weather)) %>%
  step_mutate(weather = factor(weather)) %>%
  step_date(datetime, features = c("month", "year")) %>%   # keep month/year if useful
  # create hour_of_week: Monday as start (0..167)
  step_mutate(
    hour_of_week = (as.integer(format(as.POSIXct(datetime, tz = "America/Denver"), "%u")) - 1) * 24 +
                   as.integer(format(as.POSIXct(datetime, tz = "America/Denver"), "%H"))
  ) %>%
  # cyclical transforms (period = 168 hours)
  step_mutate(
    hour_of_week_sin = sin(2 * pi * hour_of_week / 168),
    hour_of_week_cos = cos(2 * pi * hour_of_week / 168)
  ) %>%
  step_rm(hour_of_week, datetime) %>%                      # drop raw datetime and intermediate
  step_dummy(all_nominal_predictors()) %>%
  step_zv(all_predictors()) %>%
  step_lincomb(all_predictors()) %>%
  step_normalize(all_numeric_predictors())

prepped_recipe <- prep(bike_recipe)

```

## Baking (Optional if you are using a workflow)

```{r}
train_processed <- bake(prepped_recipe, new_data = train)
test_processed  <- bake(prepped_recipe, new_data = test) 
```

## Workflow

```{r}
final_wf <- workflow() |>
add_recipe(bike_recipe) |>
add_model(model) |>
fit(data=train)
```

## Tuning

```{r}
## Grid of values to tune over13
#tune_grid <- grid_regular(
 # trees(),
  #levels = 5   # or however many points you want
#)

## Split data for CV
#folds <- vfold_cv(train, v = 2)

## Run the CV
#param_tune <- tune_grid(
 # wf,
 # resamples = folds,
  #grid = tune_grid,
#  metrics = metric_set(rmse, rsq))

## Plot Results (example)
# #collect_metrics(param_tune) |> # Gathers metrics into DF
# filter(.metric=="rmse") |>
# ggplot(aes(x = trees, y = mean)) +
# geom_line() +
# geom_point() +
# labs(x = "Number of Trees",
# y = "RMSE")
# 
# ## Find Best Tuning Parameters
# best_param <- select_best(param_tune, metric="rmse")
# 
# ## Finalize the Workflow & fit it
# final_wf <- wf |>
# finalize_workflow(best_param) |>
# fit(data=train)

```

## Predictions

```{r}
## Predict
#final_wf |>
#predict(new_data = test)

test_preds <- predict(final_wf, new_data = test)
test_preds <- test_preds |>
  mutate(.pred = exp(.pred))
```

## Kaggle Submission

```{r}

kaggle_submission <- test_preds %>%
bind_cols(., test) %>% 
  select(datetime, .pred) %>%
  rename(count=.pred) %>%  
  mutate(count=pmax(0, count)) %>% 
  mutate(datetime=as.character(format(datetime)))
## Write out the file9
vroom_write(x = kaggle_submission, 
            file = "~/GitHub/Stat 348/Bike-Share/Kaggle_Submission.csv", 
            delim = ",")
```
